...
class WC_PayGol_Gateway extends WC_Payment_Gateway {
    ...
    function generate_paygol_form($orderID) {
        ...
        foreach ($paygol_args as $key => $value) {
     	...
     	return  '<form action="'.esc_url($gateway_address).'" method="post" name="pg_frm" id="pg_frm" target="_top">' . implode( '', $paygol_args_array) . '      
    ...
    function order_received($order_id){
        ...
        switch($order->get_status()){
            ...
            default:{
                //$order->update_status('processing');
                wc_print_notice( __( 'Your order is awaiting, it will be completed once it is confirmed by the local payment provider.', 'paygol_wc' ), 'notice' );          
            }
        }
